...
ybr_tab <- ybr_tab %>% 
  # Add confidence intervals to the data table summarizing results related to the model
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ yb_confint[4, 1], 
                                           term == "relative_par_load" ~ yb_confint[5, 1],
                                           term == "hatch_size" ~ yb_confint[6, 1],
                                           term == "sexM" ~ yb_confint[7, 1],
                                           term == "periodF" ~ yb_confint[8, 1],
                                           term == "relative_par_load:hatch_size" ~ yb_confint[9, 1],
                                           term == "relative_par_load:sexM" ~ yb_confint[10, 1],
                                           group == "broodID" ~ yb_confint[1, 1],
                                           group == "year" ~ yb_confint[2, 1],
                                           group == "Residual" ~ yb_confint[3, 1],
                                           TRUE ~ NA_real_),
                up95ci = dplyr::case_when(term == "(Intercept)" ~ yb_confint[4, 2], 
                                          term == "relative_par_load" ~ yb_confint[5, 2],
                                          term == "hatch_size" ~ yb_confint[6, 2],
                                          term == "sexM" ~ yb_confint[7, 2],
                                          term == "periodF" ~ yb_confint[8, 2],
                                          term == "relative_par_load:hatch_size" ~ yb_confint[9, 2],
                                          term == "relative_par_load:sexM" ~ yb_confint[10, 2],
                                          group == "broodID" ~ yb_confint[1, 2],
                                          group == "year" ~ yb_confint[2, 2],
                                          group == "Residual" ~ yb_confint[3, 2],
                                          TRUE ~ NA_real_),
                nb_groups = dplyr::case_when(group == "broodID" ~ lme4::ngrps(yb_mod)[1],
                                             group == "year" ~ lme4::ngrps(yb_mod)[2],
                                             TRUE ~ NA_integer_),
                nb_obs = dplyr::case_when(group == "Residual" ~ stats::nobs(yb_mod),
                                          TRUE ~ NA_integer_),
                exp_var = "YB")
...
yuvcr_tab <- yuvcr_tab %>% 
  # Add confidence intervals to the data table summarizing results related to the model
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ yuvc_confint[4, 1], 
                                           term == "relative_par_load" ~ yuvc_confint[5, 1],
                                           term == "hatch_size" ~ yuvc_confint[6, 1],
                                           term == "sexM" ~ yuvc_confint[7, 1],
                                           term == "periodF" ~ yuvc_confint[8, 1],
                                           term == "relative_par_load:hatch_size" ~ yuvc_confint[9, 1],
                                           term == "relative_par_load:sexM" ~ yuvc_confint[10, 1],
                                           group == "broodID" ~ yuvc_confint[1, 1],
                                           group == "year" ~ yuvc_confint[2, 1],
                                           group == "Residual" ~ yuvc_confint[3, 1],
                                           TRUE ~ NA_real_),
                up95ci = dplyr::case_when(term == "(Intercept)" ~ yuvc_confint[4, 2], 
                                          term == "relative_par_load" ~ yuvc_confint[5, 2],
                                          term == "hatch_size" ~ yuvc_confint[6, 2],
                                          term == "sexM" ~ yuvc_confint[7, 2],
                                          term == "periodF" ~ yuvc_confint[8, 2],
                                          term == "relative_par_load:hatch_size" ~ yuvc_confint[9, 2],
                                          term == "relative_par_load:sexM" ~ yuvc_confint[10, 2],
                                          group == "broodID" ~ yuvc_confint[1, 2],
                                          group == "year" ~ yuvc_confint[2, 2],
                                          group == "Residual" ~ yuvc_confint[3, 2],
                                          TRUE ~ NA_real_),
                nb_groups = dplyr::case_when(group == "broodID" ~ lme4::ngrps(yuvc_mod)[1],
                                             group == "year" ~ lme4::ngrps(yuvc_mod)[2],
                                             TRUE ~ NA_integer_),
                nb_obs = dplyr::case_when(group == "Residual" ~ stats::nobs(yuvc_mod),
                                          TRUE ~ NA_integer_),
                exp_var = "YUVC")
...
ycr_tab <- ycr_tab %>% 
  # Add confidence intervals to the data table summarizing results related to the model
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ yc_confint[4, 1], 
                                           term == "relative_par_load" ~ yc_confint[5, 1],
                                           term == "hatch_size" ~ yc_confint[6, 1],
                                           term == "sexM" ~ yc_confint[7, 1],
                                           term == "periodF" ~ yc_confint[8, 1],
                                           term == "relative_par_load:hatch_size" ~ yc_confint[9, 1],
                                           term == "relative_par_load:sexM" ~ yc_confint[10, 1],
                                           group == "broodID" ~ yc_confint[1, 1],
                                           group == "year" ~ yc_confint[2, 1],
                                           group == "Residual" ~ yc_confint[3, 1],
                                           TRUE ~ NA_real_),
                up95ci = dplyr::case_when(term == "(Intercept)" ~ yc_confint[4, 2], 
                                          term == "relative_par_load" ~ yc_confint[5, 2],
                                          term == "hatch_size" ~ yc_confint[6, 2],
                                          term == "sexM" ~ yc_confint[7, 2],
                                          term == "periodF" ~ yc_confint[8, 2],
                                          term == "relative_par_load:hatch_size" ~ yc_confint[9, 2],
                                          term == "relative_par_load:sexM" ~ yc_confint[10, 2],
                                          group == "broodID" ~ yc_confint[1, 2],
                                          group == "year" ~ yc_confint[2, 2],
                                          group == "Residual" ~ yc_confint[3, 2],
                                          TRUE ~ NA_real_),
                nb_groups = dplyr::case_when(group == "broodID" ~ lme4::ngrps(yc_mod)[1],
                                             group == "year" ~ lme4::ngrps(yc_mod)[2],
                                             TRUE ~ NA_integer_),
                nb_obs = dplyr::case_when(group == "Residual" ~ stats::nobs(yc_mod),
                                          TRUE ~ NA_integer_),
                exp_var = "YC")
