...
cowplot::save_plot("figures/not_standardized/fig3.jpg", plot_ad, ncol = 1, nrow = 1, base_height = 6, base_width = 18)
...
bb_hatch_lm <- ggplot(as.data.frame(bb_hatch_predict), 
  ...
  theme(text = element_text(family = "Noto Sans"),
        ...
        strip.background = element_blank())
ggsave("figures/not_standardized/fig3_hatch.jpg", bb_hatch_lm, width = 8, height = 5)




buvc_sex_predict = ggeffects::ggpredict(buvca_mod, terms = c("relative_par_load", "sex")) 

cols <- c("F" = "grey59", "M" = "black")


buvc_sex_lm <- ggplot(as.data.frame(buvc_sex_predict), 
                      aes(x = x, y = predicted, 
                          group = group, 
                          color = group, 
                          fill = group)) +
  geom_point(data = attr(buvc_sex_predict, "rawdata"), 
             aes(x = jitter(x),
                 y = jitter(response)),
             alpha = 0.6,
             shape = 16,
  ...
  theme(text = element_text(family = "Noto Sans"),
        ...
        legend.title = element_text(size = 12, face = "bold"),
        strip.background = element_blank())
ggsave("figures/not_standardized/fig3_sex.jpg", buvc_sex_lm, width = 8, height = 5)
...
adult_sd <- adult %>% 
  ...
  dplyr::left_join(dplyr::select(adult, 
                                 c("indID", "year", "min_age", "hatch_size", "par_load", "relative_par_load", "BB", "BUVC", "YB", "YUVC", "YC")), 
                   by = c("indID", "year")) %>% 
  dplyr::select(indID, broodID, year, sex, period,
                min_age = "min_age.y",
                sd_min_age = "min_age.x",
...
#'Building models
#'Mean brightness from the blue crown
bba_mod_sd <- lme4::lmer(sd_BB ~ sd_relative_par_load * sd_hatch_size + sd_relative_par_load * sex + sd_min_age + period + (1|year) + (1|broodID) + (1|indID), data = adult_sd)
...
bba_tab_sd <- bba_tab_sd %>% 
  ...
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ bba_confint_sd[5, 1], 
                                           term == "sd_relative_par_load" ~ bba_confint_sd[6, 1],
                                           term == "sd_hatch_size" ~ bba_confint_sd[7, 1],
                                           term == "sexM" ~ bba_confint_sd[8, 1],
                                           term == "sd_min_age" ~ bba_confint_sd[9, 1],
                                           term == "periodF" ~ bba_confint_sd[10, 1],
                                           term == "sd_relative_par_load:sd_hatch_size" ~ bba_confint_sd[11, 1],
                                           term == "sd_relative_par_load:sexM" ~ bba_confint_sd[12, 1],
                ...
                up95ci = dplyr::case_when(term == "(Intercept)" ~ bba_confint_sd[5, 2], 
                                          term == "sd_relative_par_load" ~ bba_confint_sd[6, 2],
                                          term == "sd_hatch_size" ~ bba_confint_sd[7, 2],
                                          term == "sexM" ~ bba_confint_sd[8, 2],
                                          term == "sd_min_age" ~ bba_confint_sd[9, 2],
                                          term == "periodF" ~ bba_confint_sd[10, 2],
                                          term == "sd_relative_par_load:sd_hatch_size" ~ bba_confint_sd[11, 2],
                                          term == "sd_relative_par_load:sexM" ~ bba_confint_sd[12, 2],
...
#'UV chroma from the blue crown
buvca_mod_sd <- lme4::lmer(sd_BUVC ~ sd_relative_par_load * sd_hatch_size + sd_relative_par_load * sex + sd_min_age + period + (1|year) + (1|broodID) + (1|indID), data = adult_sd)
...
buvca_tab_sd <- buvca_tab_sd %>% 
  ...
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ buvca_confint_sd[5, 1], 
                                           term == "sd_relative_par_load" ~ buvca_confint_sd[6, 1],
                                           term == "sd_hatch_size" ~ buvca_confint_sd[7, 1],
                                           term == "sexM" ~ buvca_confint_sd[8, 1],
                                           term == "sd_min_age" ~ buvca_confint_sd[9, 1],
                                           term == "periodF" ~ buvca_confint_sd[10, 1],
                                           term == "sd_relative_par_load:sd_hatch_size" ~ buvca_confint_sd[11, 1],
                                           term == "sd_relative_par_load:sexM" ~ buvca_confint_sd[12, 1],
                ...
                up95ci = dplyr::case_when(term == "(Intercept)" ~ buvca_confint_sd[5, 2], 
                                          term == "sd_relative_par_load" ~ buvca_confint_sd[6, 2],
                                          term == "sd_hatch_size" ~ buvca_confint_sd[7, 2],
                                          term == "sexM" ~ buvca_confint_sd[8, 2],
                                          term == "sd_min_age" ~ buvca_confint_sd[9, 2],
                                          term == "periodF" ~ buvca_confint_sd[10, 2],
                                          term == "sd_relative_par_load:sd_hatch_size" ~ buvca_confint_sd[11, 2],
                                          term == "sd_relative_par_load:sexM" ~ buvca_confint_sd[12, 2],
...
#'Mean brightness from the yellow breast patch
yba_mod_sd <- lme4::lmer(sd_YB ~ sd_relative_par_load * sd_hatch_size + sd_relative_par_load * sex + sd_min_age + period + (1|year) + (1|broodID) + (1|indID), data = adult_sd)
...
yba_tab_sd <- yba_tab_sd %>% 
  ...
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ yba_confint_sd[5, 1], 
                                           term == "sd_relative_par_load" ~ yba_confint_sd[6, 1],
                                           term == "sd_hatch_size" ~ yba_confint_sd[7, 1],
                                           term == "sexM" ~ yba_confint_sd[8, 1],
                                           term == "sd_min_age" ~ yba_confint_sd[9, 1],
                                           term == "periodF" ~ yba_confint_sd[10, 1],
                                           term == "sd_relative_par_load:sd_hatch_size" ~ yba_confint_sd[11, 1],
                                           term == "sd_relative_par_load:sexM" ~ yba_confint_sd[12, 1],
                ...
                up95ci = dplyr::case_when(term == "(Intercept)" ~ yba_confint_sd[5, 2], 
                                          term == "sd_relative_par_load" ~ yba_confint_sd[6, 2],
                                          term == "sd_hatch_size" ~ yba_confint_sd[7, 2],
                                          term == "sexM" ~ yba_confint_sd[8, 2],
                                          term == "sd_min_age" ~ yba_confint_sd[9, 2],
                                          term == "periodF" ~ yba_confint_sd[10, 2],
                                          term == "sd_relative_par_load:sd_hatch_size" ~ yba_confint_sd[11, 2],
                                          term == "sd_relative_par_load:sexM" ~ yba_confint_sd[12, 2],
...
#'UV chroma from the yellow breast patch
yuvca_mod_sd <- lme4::lmer(sd_YUVC ~ sd_relative_par_load * sd_hatch_size + sd_relative_par_load * sex + sd_min_age + period + (1|year) + (1|broodID) + (1|indID), data = adult_sd)
...
yuvca_tab_sd <- yuvca_tab_sd %>% 
  ...
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ yuvca_confint_sd[5, 1], 
                                           term == "sd_relative_par_load" ~ yuvca_confint_sd[6, 1],
                                           term == "sd_hatch_size" ~ yuvca_confint_sd[7, 1],
                                           term == "sexM" ~ yuvca_confint_sd[8, 1],
                                           term == "sd_min_age" ~ yuvca_confint_sd[9, 1],
                                           term == "periodF" ~ yuvca_confint_sd[10, 1],
                                           term == "sd_relative_par_load:sd_hatch_size" ~ yuvca_confint_sd[11, 1],
                                           term == "sd_relative_par_load:sexM" ~ yuvca_confint_sd[12, 1],
                ...
                up95ci = dplyr::case_when(term == "(Intercept)" ~ yuvca_confint_sd[5, 2], 
                                          term == "sd_relative_par_load" ~ yuvca_confint_sd[6, 2],
                                          term == "sd_hatch_size" ~ yuvca_confint_sd[7, 2],
                                          term == "sexM" ~ yuvca_confint_sd[8, 2],
                                          term == "sd_min_age" ~ yuvca_confint_sd[9, 2],
                                          term == "periodF" ~ yuvca_confint_sd[10, 2],
                                          term == "sd_relative_par_load:sd_hatch_size" ~ yuvca_confint_sd[11, 2],
                                          term == "sd_relative_par_load:sexM" ~ yuvca_confint_sd[12, 2],
...
#'Yellow chroma from the yellow breast patch
yca_mod_sd <- lme4::lmer(sd_YC ~ sd_relative_par_load * sd_hatch_size + sd_relative_par_load * sex + sd_min_age + period + (1|year) + (1|broodID) + (1|indID), data = adult_sd)
...
yca_tab_sd <- yca_tab_sd %>% 
  ...
  dplyr::mutate(low95ci = dplyr::case_when(term == "(Intercept)" ~ yca_confint_sd[5, 1], 
                                           term == "sd_relative_par_load" ~ yca_confint_sd[6, 1],
                                           term == "sd_hatch_size" ~ yca_confint_sd[7, 1],
                                           term == "sexM" ~ yca_confint_sd[8, 1],
                                           term == "sd_min_age" ~ yca_confint_sd[9, 1],
                                           term == "periodF" ~ yca_confint_sd[10, 1],
                                           term == "sd_relative_par_load:sd_hatch_size" ~ yca_confint_sd[11, 1],
                                           term == "sd_relative_par_load:sexM" ~ yca_confint_sd[12, 1],
                ...
                up95ci = dplyr::case_when(term == "(Intercept)" ~ yca_confint_sd[5, 2], 
                                          term == "sd_relative_par_load" ~ yca_confint_sd[6, 2],
                                          term == "sd_hatch_size" ~ yca_confint_sd[7, 2],
                                          term == "sexM" ~ yca_confint_sd[8, 2],
                                          term == "sd_min_age" ~ yca_confint_sd[9, 2],
                                          term == "periodF" ~ yca_confint_sd[10, 2],
                                          term == "sd_relative_par_load:sd_hatch_size" ~ yca_confint_sd[11, 2],
                                          term == "sd_relative_par_load:sexM" ~ yca_confint_sd[12, 2],
...
#'Order and level factors to organize the forest plot
ad_tab_sd$exp_var <- factor(ad_tab_sd$exp_var, c("BB_sd", "BUVC_sd", "YB_sd", "YUVC_sd", "YC_sd"))
ad_tab_sd$term <- factor(ad_tab_sd$term, c("sd_relative_par_load:sd_hatch_size", "sd_hatch_size", "sd_relative_par_load:sexM",
                                           "sd_relative_par_load", "sd_min_age", "periodF", "sexM", "(Intercept)"))
...
plot_ad_sd <- cowplot::ggdraw() +
  ...
  cowplot::draw_text("Yellow breast patch", x = 0.75, y = 0.96, size = 14, fontface = "bold", family = "Noto Sans", color = "darkorange") +
  cowplot::draw_line(x = c(0.55, 0.9), y = c(0.93, 0.93), linewidth = 1, color = "black")
cowplot::save_plot("figures/standardized/fig3_sd.jpg", plot_ad_sd, ncol = 1, nrow = 1, base_height = 6, base_width = 18)



# Plot displaying results from model on mean brightness from the blue crown
bb_hatch_predict_sd = ggeffects::ggpredict(bba_mod_sd, terms = c("sd_relative_par_load", "sd_hatch_size")) 
...
ggsave("figures/standardized/fig3_lmbb_sd.jpg", bb_hatch_lm_sd, width = 8, height = 5)



# Plot displaying results from model on mean brightness from the blue crown
buvc_sex_predict_sd = ggeffects::ggpredict(buvca_mod_sd, terms = c("sd_relative_par_load", "sex")) 
...
ggsave("figures/standardized/fig3_buvcsex_sd.jpg", buvc_sex_lm_sd, width = 8, height = 5)


#Display table with output (not standardized and standardized)
##not standardized
...
colnames(ad_tab) <- name
write.csv(ad_tab, "output_tables/output_prediction2_unst.csv", row.names = FALSE)

##standardized
ad_tab_sd <- dplyr::bind_rows(bba_tab_sd, buvca_tab_sd, yba_tab_sd, yuvca_tab_sd, yca_tab_sd) %>% 
  dplyr::mutate_at(vars(estimate, std.error, statistic, low95ci, up95ci), ~ round(., digits = 4)) %>% # Round numbers to keep only 4 digits
  dplyr::select(exp_var, effect, term, estimate, std.error, statistic, low95ci, up95ci, group, nb_obs, nb_groups) # Reduce table width to the needed columns
colnames(ad_tab_sd) <- name
write.csv(ad_tab_sd, "output_tables/output_prediction2_st.csv", row.names = FALSE)